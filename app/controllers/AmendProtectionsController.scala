...
trait AmendProtectionsController extends BaseController {
  ...
  def amendProtection(nino: String, id: Integer) = WithCitizenRecordCheck(nino).async(BodyParsers.parse.json) { implicit request =>
    ...
    protectionAmendmentJs.fold(
      ...
      p => protectionService.amendProtection(nino, id, request.body.as[JsObject]) map { response =>
